...
class InactiveTrainerListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    '''
    Overview of all users of type trainer in the instance
    '''
    model = User
    permission_required = ('gym.manage_gyms',)
    template_name = 'user/inactive_trainers.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        Pass other info to the template
        '''
        context = super(InactiveTrainerListView, self).get_context_data(**kwargs)
        context['show_gym'] = True
        context['user_table'] = {'keys': [_('ID'),
                                          _('Username'),
                                          _('Name'),
